...
A continuación, la Figura \ref{almaarray1} muestra la distribución de antenas para el muestro del objeto HLTauri en banda 6. Además la Figura \ref{almaarray2} muestra el centro de la primera figura en donde se puede visualizar una gran concentración de antenas. Además, se puede ver que cada antena posee un identificador (DA, DV o PM) dando a conocer el tipo de antena que está siendo utilizado para el muestreo.
...
Por ejemplo, la estrella HLTauri ha sido muestreada en banda 3, banda 6 y banda 7.


\begin{figure}[h!]
\centering
\subfloat[Banda 3]{\includegraphics[scale=0.12]{images/HLTau_B3total.png}}%
\subfloat[Banda 6]{\includegraphics[scale=0.12]{images/HLTau_B6total.png}}%
\subfloat[Banda 7]{\includegraphics[scale=0.12]{images/HLTau_B7total.png}}%
\label{fig:HLTau-total}
\caption{Mapeos $(u,v)$ del objeto HLTauri (HLTau) en diferentes bandas. Fuente: Elaboración propia}
\end{figure}



Luego de que las ondas milimétricas y submilímetricas son recibidas por las antenas, éstas deben ser discretizadas y llevadas al correlacionador. El correlacionador recibe señales de voltaje desde cada antena, calcula la correlación cruzada y autocorrelación para éstas por cada par de antenas (\textit{baselines}), y produce las visibilidades complejas que los astrónomos reciben para sintetizar imágenes \citep{alma-handbook}. Para entender de mejor forma qué son las visibilidades y qué implicancia tienen en la síntesis de imágenes es necesario conocer los conceptos esenciales de interferometría.
